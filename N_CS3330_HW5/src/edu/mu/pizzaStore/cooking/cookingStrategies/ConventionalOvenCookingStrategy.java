...
public class ConventionalOvenCookingStrategy implements ICookingStrategy{
	...
	private final double cookingPrice = 8.0;

	@Override
	public boolean cook(AbstractPizza pizza) {
		if(pizza.getCookingStrategy() == new ConventionalOvenCookingStrategy()) {
			System.out.println("This cooking style is already selected");
			return false;
			}
		if(pizza.getCookingStrategy() != null) {
			System.out.println("Changing current cooking style from: " + pizza.getCookingStrategy() + " to Conventional Oven");
		}
		try {
